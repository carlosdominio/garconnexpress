...
module.exports = (ctx) => {
  ...
  // GET /api/config/ordem-categorias
  router.get('/config/ordem-categorias', async (req, res) => {
    try {
  ...
  router.post('/config/ordem-categorias', isAdmin, async (req, res) => {
    ...
    try {
      ...
      if (ctx) ctx.cachedOrdemCategorias = Array.isArray(ordem) ? ordem.map(c => String(c).trim().toUpperCase()) : null;
