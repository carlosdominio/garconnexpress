               ...
               if (admin.apps.length > 0) {
                 let activeSound = 'notificacao';
                  ...
                  if (targetApp === 'garcom') {
                    ...
                    channelName = 'churrasqueiro_canal_' + activeSound + '';
                  }
...
async function checkAndNotifyDelayedOrders() {
  ...
  try {
      ...
      for (const sub of subs) {
        ...
        if (!isNativeSubAtraso) {
          ...
          if (admin.apps.length > 0) {
            ...
            const channelName = 'garcom_canal_' + activeSound + '';
            let fcmSoundFile = activeSound;
            if (fcmSoundFile === 'original') fcmSoundFile = 'notificacao';
    ...
    for (const p of delayedOrders) {
      ...
      // Dispara o WhatsApp diretamente do servidor (não bloqueia notificações de tela/push em caso de erro/falha no WhatsApp)
      let dateStr = p.created_str || '';
      if (!dateStr.endsWith('Z')) dateStr = dateStr.replace(' ', 'T') + 'Z';
      const createdAt = new Date(dateStr);
  ...
  try {
    ...
    for (const sub of subs) {
      ...
      if (!isNativeSub) {
        ...
        if (admin.apps.length > 0) {
          const channelName = 'garcom_canal_' + fcmSoundFile + '';
          const androidNotification = {
            channelId: channelName,
            defaultSound: activeSound === 'original',
            notificationPriority: 'PRIORITY_MAX'
...
app.use('/app-churrasqueiro', express.static(path.join(__dirname, 'churrasqueiro-app-nativo', 'www'), noCacheHeaders));
...
async function getFilterCozinha() {
  ...
  return `(
    CASE 
      WHEN m.enviar_churrasco = ${sqlTrue} THEN 0
      WHEN m.enviar_cozinha = ${sqlTrue} THEN 1
      WHEN m.enviar_cozinha = ${sqlFalse} THEN 0
      WHEN m.enviar_churrasco = ${sqlFalse} AND ${matchCozinhaCat} THEN 1
      WHEN ${isChurrascoCat} THEN 0
      WHEN ${matchCozinhaCat} THEN 1
      ELSE 0
    END = 1
  )`;
}
...
async function getFilterChurrasco() {
  try {
    ...
    return `(
      CASE 
        WHEN m.enviar_churrasco = ${sqlTrue} THEN 1
        WHEN m.enviar_churrasco = ${sqlFalse} THEN 0
        WHEN ${fallbackCheck} THEN 1
        ELSE 0
      END = 1
    )`;
  } catch (err) {
  ...
  try {
    ...
    if (temPendente) {
      if (statusAnterior !== 'recebido') {
        await query("UPDATE pedidos SET total = ?, status = ?, created_at = ?, observacao = ?, fechamento_liberado = FALSE, fechamento_solicitado_em = NULL, solicitou_fechamento = FALSE, notificado_atraso_fechamento = 0 WHERE id = ?", [total, novoStatusPedido, agora, observacao || '', id]);
      } else {
        await query("UPDATE pedidos SET total = ?, status = ?, observacao = ?, fechamento_liberado = FALSE, fechamento_solicitado_em = NULL, solicitou_fechamento = FALSE, notificado_atraso_fechamento = 0 WHERE id = ?", [total, novoStatusPedido, observacao || '', id]);
      }
...
app.put('/api/pedidos/:id/adicionar', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    // Se já estava em 'recebido', mantém o original.
    if (statusAnterior !== 'recebido') {
      await query("UPDATE pedidos SET total = ?, cobrar_taxa = ?, status = 'recebido', created_at = ?, observacao = ?, fechamento_liberado = FALSE, fechamento_solicitado_em = NULL, solicitou_fechamento = FALSE, notificado_atraso_fechamento = 0 WHERE id = ?", [tot, isPostgres ? deveTaxa : (deveTaxa?1:0), agora, observacao || '', id]);
    } else {
      await query("UPDATE pedidos SET total = ?, cobrar_taxa = ?, status = 'recebido', observacao = ?, fechamento_liberado = FALSE, fechamento_solicitado_em = NULL, solicitou_fechamento = FALSE, notificado_atraso_fechamento = 0 WHERE id = ?", [tot, isPostgres ? deveTaxa : (deveTaxa?1:0), observacao || '', id]);
    }
