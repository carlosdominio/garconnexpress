...
async function carregarPedidos(opcoes = {}) {
    ...
    try {
        ...
        if (container && container.innerHTML.includes('Carregando pedidos...')) {
            renderizarPedidos([]);
            if (typeof Swal !== 'undefined') Swal.fire('Erro', 'Não foi possível carregar os pedidos. Verifique sua conexão.', 'error');
        }
  ...
  try {
    ...
    if (window.Capacitor.getPlatform() === 'android') {
      ...
      for (const som of todosOsSons) {
        try {
          await PushNotifications.createChannel({
            id: 'cozinha_canal_' + som,
            name: 'Cozinha - ' + som.replace(/_/g, ' '),
      ...
      console.log('✅ Todos os canais FCM da Cozinha registrados no Android.');
    }
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            if (typeof exibirTelaCarregamentoSistema === 'function') {
              exibirTelaCarregamentoSistema('⚡ Aplicativo Bloqueado', 'Baixando nova versão do sistema. Instale o APK baixado para poder voltar a utilizar o CozinhaExpress.');
            }
          }
        });
      }
