...
async function verificarVersaoSistema() {
  try {
    ...
    if (data && data.versao && data.versao !== CLIENT_VERSION) {
      if (typeof pedidoAtual !== 'undefined' && pedidoAtual.length > 0) {
        console.log('⚠️ Nova versão disponível, mas aguardando esvaziamento do carrinho para recarregar.');
        return;
      }
...
async function configurarPusher() {
  try {
    ...
    channel.bind('rascunho-cancelado', (data) => {
      ...
      if (data.cancelado_pelo_cliente && numMesa) {
        mostrarToast(`O rascunho da Mesa ${numMesa} foi cancelado pelo cliente.`, "info", "🚫 RASCUNHO CANCELADO");
      } else if (numMesa) {
        mostrarToast(`O rascunho da Mesa ${numMesa} foi processado/cancelado.`, "info");
      }
      carregarMesas();
    });
