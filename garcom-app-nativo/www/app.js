...
// REGISTRO DE SERVICE WORKER (PWA) E WEB PUSH (MANTIDO COMO FALLBACK, MAS O FOCO AGORA É NATIVO)
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('/garcom/sw.js')
      .then(reg => {
        console.log('🚀 Service Worker registrado (Fallback Web)!', reg);
      })
...
async function registerNativePush() {
  try {
    ...
    // 1. Solicita permissão ao usuário (Android 13+)
    ...
    // 2. Registra o dispositivo no Firebase para receber o token
    await PushNotifications.register();

    // 3. Ouvintes de eventos do Push
    PushNotifications.addListener('registration', async (token) => {
      console.log('🔥 Token FCM recebido:', token.value);
      // Envia o token para o nosso backend
      await fetch('/api/subscribe', {
        ...
        body: JSON.stringify({
          endpoint: token.value, // O backend vai identificar que é nativo pela ausência de "keys" e formato
          keys: { p256dh: '', auth: '' }
        })
      });
      console.log('✅ Token FCM registrado no servidor com sucesso!');
    });

    PushNotifications.addListener('registrationError', (error) => {
      console.error('❌ Erro ao registrar Push FCM:', JSON.stringify(error));
    });

    // Quando o app está aberto e recebe uma notificação (Opcional, pois já usamos o Pusher para real-time quando aberto)
    PushNotifications.addListener('pushNotificationReceived', (notification) => {
      console.log('📩 Notificação nativa recebida (App Aberto):', notification);
      carregarMesas(); // Força atualização da tela
    });

    // Quando o usuário clica na notificação na barra do sistema
    PushNotifications.addListener('pushNotificationActionPerformed', (notification) => {
      console.log('👆 Usuário clicou na notificação nativa:', notification);
      carregarMesas();
    });

  } catch (error) {
    console.error('❌ Erro ao configurar Push Nativo (Capacitor):', error);
  }
}
// ------------------------------------

// Mantido apenas se por algum motivo for rodar no navegador (não nativo)
...
async function subscribeToPush() {
  if (isNativeApp) return; // Se for nativo, não tenta usar o VAPID
